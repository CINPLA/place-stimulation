...
def plot_rate_map(x, y, t, sptr, boxsize=1, binsize=0.02, smoothing=0.02, ax=None):
    map = sm.SpatialMap(x, y, t, sptr, boxsize, binsize)
    rate_map = map.rate_map(smoothing=smoothing, mask_zero_occupancy=True)
...
def plot_path(x, y, t, sptr=None, figsize=[5, 5], ax=None, s=30, c=[0.7, 0.2, 0.2], scatter=True):
...
def plot_split_path(x, y, t, sptr=None, ax=None, figsize=(3, 20),  s=30, c=[0.7, 0.2, 0.2], scatter=True):
    if ax is None:
        fig = plt.figure(figsize=figsize)
        ax = fig.add_subplot(111)
        # sns.set(color_codes=True, style="darkgrid")
    ...
    angle_position = np.arctan(y_center / x_center)
    angle_spikes = np.arctan(y_spike / x_spike)

    ax.plot(angle_position, t, 'k', alpha=0.3)
    ax.scatter(angle_spikes, sptr_t, s=s, c=c)
